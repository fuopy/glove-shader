...
// PROMPT ////////////////////////////////////////////////////////////////////
void update_prompt(inout Prompt obj)
{
	if (A_PRESSED)
	{
		if (obj.inputBufferLength >= MAX_PROMPT_INPUT_BUFFER)
		{
			// Exit prompt.
			obj.active = false;
		}
		else
		{
			...
			for (int i = 0; i < MAX_PROMPT_INPUT_BUFFER; ++i) // TODO: "Array reference can not be used as an l-value", this is a weird workaround because the simple copy approach doesn't work.
			{
				if (i == obj.inputBufferLength)
				{
					obj.inputBuffer[i] = keyboardReturnString[obj.cursor];
					obj.inputBufferLength++;
					break;
				}
			}
	...
	{
        obj.cursor += obj.tabCount;
		if (obj.cursor >= obj.tabCount * obj.rows)
		{
			obj.cursor -= obj.tabCount * obj.rows;
		}
		sound_Move = PLAY_SOUND;
